...
public interface WeddingHallDocs {
    ...
    @Operation(summary = "내 웨딩홀 목록", description = "오너 본인 소유 상품 (최신순, 페이지네이션)")
    @ApiResponse(responseCode = "200", description = "조회 성공",
    ...
    @Operation(
            summary = "웨딩홀 필터 조회",
            description = "태그/카테고리/지역/가격범위/정렬 기준을 조합해 목록 조회 (무한스크롤용 페이지네이션 포함).",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "조회 성공",
    ...
    ResponseEntity<Page<WeddingHallPageResponse>> getWeddingHallsByTags(
            @Parameter(
                    ...
                    schema = @Schema(
                            type = "string",
                            allowableValues = {
                                    "GENERAL","CONVENTION","HOTEL","HOUSE","RESTAURANT",
                                    "HANOK","CHURCH","CHAPEL","SMALL","OUTDOOR_GARDEN","TRADITIONAL_WEDDING"
                            }
                    )
            )
            ...
            @Parameter(description = "지역", required = false)
            ...
            @Parameter(
                    description = "정렬 기준",
                    schema = @Schema(type = "string", allowableValues = {"PRICE_ASC","PRICE_DESC","POPULAR","LATEST"})
            )
