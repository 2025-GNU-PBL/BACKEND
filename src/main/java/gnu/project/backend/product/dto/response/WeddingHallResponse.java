...
public record WeddingHallResponse(
    ...
    public static WeddingHallResponse from(final WeddingHall hall) {
        return new WeddingHallResponse(
            ...
            hall.getRegion(),
            hall.getOwner().getBzName(),
            hall.getImages().stream()
                .map(img -> new ImageResponse(img.getId(), img.getUrl(), img.getDisplayOrder(),
                    img.isThumbnail()))
                .toList(),
            hall.getOptions().stream()
                .map(opt -> new OptionResponse(opt.getId(), opt.getName(), opt.getPrice(),
                    opt.getDetail()))
                .toList(),
